...
(function (factory) {
  ...
  prototype.build = function () {
	...
	if(this.options.zoomSlider) {
		...
		var minRatio = Math.min(this.options.data.width/this.image.naturalWidth, this.options.data.height/this.image.naturalHeight);
		var times = Math.log(minRatio)/Math.log(1/1.1)
		if(times > 0) {
			SLIDER_MIN = -Math.ceil(times/10);
		}
		if(isNaN(SLIDER_MIN))
			SLIDER_MIN = -1;		
		...
		$this.parent().append("<input type='range' id='zoomSlider' value='0' min='" + SLIDER_MIN + "' max='" + SLIDER_MAX + "' step='0.1'><input type='hidden' id='zoomSliderPrevValue' value='0' />");
		$("#zoomSlider").on("input", function() 
		{
			var ratio = this.value - ($("#zoomSliderPrevValue").val());
			ratio = Math.round( ratio * 10 ) / 10;
			
			self.zoom(ratio); 
			$("#zoomSliderPrevValue").val(this.value)
		});
	}
    this.addListeners();
    this.initPreview();
  ...
  $.extend(prototype, {
    ...
    wheel: function (event) {
	  ...
	  if(this.options.zoomSlider) {
		...
		$("#zoomSlider").val($("#zoomSlider").val() - delta * 0.1);
	  }
      this.zoom(-delta * 0.1);
	  
	  if(this.options.zoomSlider) {
	    $("#zoomSliderPrevValue").val($("#zoomSlider").val());
	  }
    },
  ...
  $.extend(prototype, {
    ...
    zoom: function (delta) {
		...
		if(this.options.zoomSlider)
		{
			var currentZoomValue = $("#zoomSlider").val();
			var prevZoomValue = $("#zoomSliderPrevValue").val();
			
			if(prevZoomValue == currentZoomValue && ((currentZoomValue >= SLIDER_MAX && delta > 0) || (currentZoomValue <= SLIDER_MIN && delta < 0)))
				return;
      ...
      if ((delta) && this.built && !this.disabled && this.options.zoomable) {
		...
		delta = delta < 0? 1/(1-delta) : 1+delta;
