(function () {
    ...
    timezoneService.$inject = ['authService'];

    function timezoneService(authService) {
        ...
        var service = {
            ...
            currentDateTimeInTimeZone: currentDateTimeInTimeZone,
            getLast24HoursTimeTable: getLast24HoursTimeTable,
            formatMomentForAnalyticsChart: formatMomentForAnalyticsChart
        ...
        function formatDateTime(dt){
            var timezone = authService.getCurrentUser().timezone;
            var userTimeZone = (timezone!=null && timezone!="" && typeof(timezone) !='undefined') ? timezone : "Australia/Sydney";// todo: get user's timezone
            if(typeof(dt) === 'undefined')
            return moment().tz(userTimeZone).format('DD/MM/YYYY HH:mm');
            else
            return moment(dt).tz(userTimeZone).format('DD/MM/YYYY HH:mm');
        }

        function convertToUtc(dt){
            return moment.utc(dt).format('YYYY-MM-DD HH:mm:ss Z');
        }
        ...
        function convertToLocal(dt){
            ...
            return moment(dt).tz(userTimeZone).format('YYYY-MM-DD HH:mm:ss Z');
        }

        function currentDateTimeInTimeZone(){
            var timezone = authService.getCurrentUser().timezone;
            var userTimeZone = (timezone!=null && timezone!="" && typeof(timezone) !='undefined') ? timezone : "Australia/Sydney";// todo: get user's timezone
        ...
        function getLast24HoursTimeTable(fromTime){
            var timezone = authService.getCurrentUser().timezone;
            var userTimeZone = (timezone!=null && timezone!="" && typeof(timezone) !='undefined') ? timezone : "Australia/Sydney";// todo: get user's timezone
            var timeTable = [];
           // for(var i=48; i>=0; i=i-1){
            for(var i=0; i<=48; i=i+1){
                timeTable.push({moment: moment(fromTime).tz(userTimeZone).add(i,'hours').format("DD MMM, hA"), opened: 0, clicked: 0});
                
            }
            return timeTable;
        }

        function formatMomentForAnalyticsChart(dt){
            var timezone = authService.getCurrentUser().timezone;
            var userTimeZone = (timezone!=null && timezone!="" && typeof(timezone) !='undefined') ? timezone : "Australia/Sydney";// todo: get user's timezone
            return moment(dt).tz(userTimeZone).format("DD MMM, hA");
        }
