(function() {
 ...
 //template += '<label for="country">{{countryLabel}}</label>';
    template += '<select  name="country" ng-change="selectCountry()" ng-model="country"  required  class="c-signatures-new__address-input c-input select_override">';
    template +='<option value="" selected>Please Select Country</option>';
    template +='<option ng-repeat="theCountry in countries track by $index" >{{theCountry}}</option>';
    template +='</select>';
    // template += '<jibbar-select style="width:100%; padding-bottom: 2px;" class="c-signatures-new__company-input c-signatures-new__address-input" select-options="countries" ';
    // template += 'select-empty-value="Country"';
    // template += 'ng-model="country" ng-change="selectCountry()"></jibbar-select>';
    template +='</div>';
    
    template +='<div class="bloc-state">';
    template +='<label for="state" ng-show="states&&showLabel" class="c-form-group__label">State</label>';
    template +='<select name="state" ng-model="countryState"  ng-show="states"  select-empty-value="States" required  class="c-signatures-new__address-input c-input select_override" >';
    template +='<option value="" selected>Please Select State</option>';
    template +='<option ng-repeat="optStates in states track by $index" >{{optStates}}</option>';
    template +='</select>';
    // template += '<jibbar-select style="width:100%; padding-bottom: 2px;" class="c-signatures-new__company-input c-signatures-new__address-input" select-options="states" ';
    // template += 'select-empty-value="State" ';
    // template += 'ng-model="countryState"></jibbar-select>';
    template +='</div>';
    ...
    angular
        ...
        .directive("countryStateSelect", [ function () {
            return {
                restrict: "E",
                template: template,
                scope: { country: "=?", countryState: "=?state" , defaultCountry:"@", selectEmptyValue: "<", showLabel: "=?labelshow"},
                link: function (scope, element, attrs) {
                    ...
                    jibbar.event.on(jibbar.event.events.COUNTRY_LOADED_EVENT, function(data) {  
                      scope.country = data; 
                      scope.selectCountry();
                   });

                    scope.selectCountry = function(){
                        if(typeof scope.country == 'undefined' || scope.country=="") return;
